// ================================
// LEVRON PORTFOLIO - JAVASCRIPT (v5)
//
// CAMBIOS v5:
// 1. MOBILE MENU — Lógica actualizada para el nuevo HTML del menú
//    premium (.mm-link en lugar de .mobile-link).
//
// 2. SMOOTH SCROLL — Reescrito con getBoundingClientRect en tiempo
//    real para máxima precisión. Calcula el offset de la nav en
//    cada llamada y nunca acumula errores en páginas largas.
//    Usa requestAnimationFrame para garantizar que el DOM esté
//    estabilizado antes del cálculo.
//
// 3. FAQ ACCORDION — Una pregunta abierta a la vez.
//
// 4. PRICING REVEAL — Usa clase CSS .pricing-reveal (sin inline styles).
//
// 5. Form prices actualizados: Esencial €500, Pro €900, Premium €1600.
// ================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openMenu = () => {
        ...
        requestAnimationFrame(() => {
            const firstLink = mobileMenu.querySelector('.mm-link');
    ...
    if (mobileMenuBtn && mobileMenu) {
        ...
        // Links del menú premium (.mm-link y .mm-cta)
        mobileMenu.querySelectorAll('.mm-link, .mm-cta').forEach(link => {
            link.addEventListener('click', (e) => {
                const href   = link.getAttribute('href');
                const target = href && href !== '#' ? document.querySelector(href) : null;
                if (target) {
                    e.preventDefault();
                    closeMenu(() => smoothScrollTo(target));
                } else {
                    closeMenu();
                }
            });
        ...
        mobileMenu.addEventListener('keydown', e => {
            ...
            const focusable = Array.from(mobileMenu.querySelectorAll('a, button')).filter(el => !el.closest('.mm-bg-deco'));
    ...
    // Calcula el offset real de la nav en cada llamada.
    // Usa doble rAF para garantizar que el layout esté
    // estabilizado antes de medir (especialmente en iOS).
    // ================================

    function smoothScrollTo(target) {
        if (!target) return;

        // Doble requestAnimationFrame: el primero programa el cálculo,
        // el segundo garantiza que el layout del frame anterior se ha
        // aplicado completamente antes de medir.
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                const nav       = document.querySelector('.nav');
                const navHeight = nav ? nav.getBoundingClientRect().height : 72;
                const rect      = target.getBoundingClientRect();
                const targetTop = Math.max(0, rect.top + window.scrollY - navHeight - 12);
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        // Los links del menú móvil se gestionan arriba — los excluimos
    ...
    const nav = document.querySelector('.nav');
    if (nav) {
        const updateNav = () => nav.classList.toggle('scrolled', window.scrollY > 100);
    ...
    // Solo añade la clase CSS — no toca opacity/transform inline.
    // ================================

    const pricingObserver = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('pricing-reveal');
...
function initMultiStepForm() {
    ...
    form.innerHTML = `
        ...
        <div class="prism-step-panel active" id="step-1">
            ...
            <div class="form-group" id="grp-fname">
                ...
                <input type="text" id="fname" name="name" autocomplete="name" placeholder="Tu nombre o el de tu negocio">
                <span class="field-error" id="err-fname"></span>
            </div>
            <div class="form-group" id="grp-femail">
                <label for="femail">Email</label>
                <input type="email" id="femail" name="email" autocomplete="email" placeholder="tucorreo@email.com">
        ...
        <div class="prism-step-panel" id="step-2">
            ...
            <div class="plan-cards">
                <div class="plan-card" data-value="esencial">
        ...
        <div class="prism-step-panel" id="step-3">
            ...
            <div class="form-group" id="grp-fmessage">
                ...
                <textarea id="fmessage" name="message" rows="4" placeholder="¿A qué te dedicas? ¿Qué quieres conseguir? Cuanto más me cuentes, mejor propuesta puedo hacerte."></textarea>
        ...
        <div class="prism-success" id="prismSuccess" aria-live="polite">
            <svg class="success-prism" viewBox="0 0 80 80" fill="none" aria-hidden="true">
                <defs><linearGradient id="sg1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#6366f1"/><stop offset="50%" stop-color="#8b5cf6"/><stop offset="100%" stop-color="#C0C0C0"/></linearGradient></defs>
            ...
            <div class="success-meta">
                <div class="success-badge"><span class="badge-val">&lt; 24h</span><span class="badge-label">Respuesta</span></div>
                <div class="success-divider" aria-hidden="true"></div>
                <div class="success-badge"><span class="badge-val">0€</span><span class="badge-label">Compromiso</span></div>
                <div class="success-divider" aria-hidden="true"></div>
                <div class="success-badge"><span class="badge-val">7 días</span><span class="badge-label">Entrega</span></div>
            </div>
        </div>
    `;
    ...
    const PLAN_LABELS = {
        esencial: 'Esencial — Landing page (€500)',
        pro:      'Pro — Web completa (€900)',
        premium:  'Premium — E-commerce (€1.600)',
    ...
    function buildSummary() {
        ...
        summary.innerHTML = `
            ...
            <div class="summary-row"><span class="summary-key">Email</span><span class="summary-val">${email}</span></div>
            <div class="summary-row"><span class="summary-key">Plan</span><span class="summary-val plan-highlight">${PLAN_LABELS[selectedPlan] || '—'}</span></div>`;
    }
    ...
    $('nextBtn2').addEventListener('click', () => {
        if (!selectedPlan) { $('planError').classList.add('visible'); return; }
        buildSummary();
        goToStep(3);
    });
