// ================================
// LEVRON PORTFOLIO - JAVASCRIPT (v3 — SCROLL FIX)
//
// FIXES v3:
// 1. SCROLL TRABADO — La causa principal era el IntersectionObserver
//    de secciones que aplicaba opacity:0 + transform inline vía JS,
//    lo que bloqueaba el repaint y causaba jank visible en móvil.
//    SOLUCIÓN: Las secciones usan clase CSS .section-hidden en lugar
//    de estilos inline. Así el navegador puede optimizar el repaint.
//
// 2. MENÚ MÓVIL — Reemplazado position:fixed en body por
//    overscroll-behavior:contain en el propio overlay. Evita el
//    recálculo de layout que causaba el scroll jump en iOS.
//
// 3. SMOOTH SCROLL — Reescrito con requestAnimationFrame para
//    que el offset de la nav se calcule correctamente en cada llamada
//    y no interfiera con secciones animadas.
//
// 4. PRECIOS actualizados: Esencial €400, Pro €900, Premium €1600.
// ================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openMenu = () => {
        ...
        // FIX v4: position:fixed en body es necesario en iOS para bloquear scroll.
        // Guardamos scrollY y lo aplicamos como top negativo para que la página
        // no salte al top mientras el menú está abierto.
    ...
    const closeMenu = (callback) => {
        ...
        // FIX v4: restaurar scroll — position:fixed resetea al top,
        // scrollTo con 'instant' lo devuelve a donde estaba sin animación visible
    ...
    // ================================
    // SMOOTH SCROLL
    // FIX v3: se recalcula el navHeight en cada llamada para que sea
    // correcto tras resize. Se usa getBoundingClientRect + scrollY
    // para que funcione aunque el elemento esté en medio de una
    // animación CSS (opacity/transform no afecta a getBoundingClientRect).
    // ================================
    ...
    function smoothScrollTo(target) {
        ...
        // Forzar un frame para que el layout esté actualizado
        requestAnimationFrame(() => {
            const rect      = target.getBoundingClientRect();
            const targetTop = rect.top + window.scrollY - navHeight - 8;
            window.scrollTo({ top: Math.max(0, targetTop), behavior: 'smooth' });
        });
    }

    // Links de la nav principal
    ...
    // ================================
    // SECTION REVEAL
    // FIX v3: En lugar de aplicar opacity:0 y transform inline (que
    // bloquea el repaint y traba el scroll), añadimos la clase CSS
    // .section-hidden que lo gestiona de forma declarativa. El
    // navegador puede aplicar compositing sin re-layout.
    // ================================
...
// ================================
// MULTI-STEP CONTACT FORM
// Precios actualizados: Esencial €400 · Pro €900 · Premium €1600
// ================================
...
function initMultiStepForm() {
    ...
    // PRECIOS ACTUALIZADOS v3
    const PLAN_LABELS = {
        esencial: 'Esencial — Landing page (€400)',
